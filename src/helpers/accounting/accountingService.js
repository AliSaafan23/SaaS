...
export const getAccountBalance = async ({ tenantId, accountCode, asOf, transaction }) => {
    const account = await Account.findOne({ where: { tenantId, code: accountCode }, transaction });
    if (!account) return 0;
    ...
    const lines = await JournalLine.findAll({
        include: [{ model: JournalEntry, as: 'entry', where: whereEntry, attributes: [] }],
