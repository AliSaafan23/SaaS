...
const REPORT_DIR = 'customer-reports';
...
const reportLabels = {
    ar: {
        receivablesTitle: 'التقرير : ذمم العملاء',
        receivablesSubtitle:
            'ملاحظة: الأرقام أدناه تمثل مبالغ عليهم على العملاء (مديونين للمحل) — وليست أرصدة لصالح العميل. تشمل الرصيد الافتتاحي (عليه) وفواتير الأجل غير المسددة بعد خصم المدفوعات.',
        withBalancesTitle: 'التقرير : العملاء عليهم ذمم متبقية',
        withBalancesSubtitle:
            'ملاحظة: يعرض هذا التقرير العملاء الذين عليهم فلوس للمحل فقط (ذمم مستحقة التحصيل). لا يشمل العملاء الذين لهم رصيد (له) أو من سددوا بالكامل.',
        date: 'التاريخ',
        time: 'الوقت',
        index: '#',
        name: 'الإسم',
        remainingCredit: 'الباقي من فواتير الأجل (عليه)',
        remainingOpening: 'الباقي من الرصيد الإفتتاحي (عليه)',
        total: 'إجمالي الذمم (عليه)',
        phone: 'رقم الهاتف',
        totalRemaining: 'إجمالي الذمم على العميل (عليه)',
        grandTotal: 'الأجمالي',
        noData: 'لا يوجد عملاء عليهم ذمم متبقية',
    },
    en: {
        receivablesTitle: 'Report: Customer Receivables',
        receivablesSubtitle:
            'Note: Amounts below are owed by customers to the store (receivables/debit), not credit balances in the customer\'s favor. Includes opening debit and unpaid credit invoices minus payments.',
        withBalancesTitle: 'Report: Customers With Outstanding Debts',
        withBalancesSubtitle:
            'Note: This report lists only customers who still owe money to the store. Excludes customers with credit balance (payable to them) or fully paid accounts.',
        date: 'Date',
        time: 'Time',
        index: '#',
        name: 'Name',
        remainingCredit: 'Unpaid credit invoices (owed)',
        remainingOpening: 'Remaining opening debit (owed)',
        total: 'Total receivables (owed)',
        phone: 'Phone',
        totalRemaining: 'Total owed by customer',
        grandTotal: 'Grand total',
        noData: 'No customers with outstanding debts',
    },
};
...
const RECEIVABLES_HTML = `<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head><meta charset="utf-8" /><style>${baseStyles}</style></head>
<body>
  <div class="header">
    <div class="title">{{labels.receivablesTitle}}</div>
    <div>{{meta.companyName}}{{#if meta.branchName}} — {{meta.branchName}}{{/if}}</div>
  </div>
  <div class="subtitle">{{labels.receivablesSubtitle}}</div>
  <div class="meta">
...
const buildPdfFile = async ({ html, data, prefix }) => {
    ...
    return {
        fileName,
        filePath,
        fileUrl: `${sharedVariable.address}${sharedVariable.customerReports}${fileName}`,
    };
};

export const generateReceivablesPdf = async (req, { items, totals }, lang = 'ar') => {
