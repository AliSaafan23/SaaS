...
const handleImgUpload = async (request, dir, subdir) => {
  ...
  let imagePath = pathUpload;
  let fileUpload = new Resize(imagePath);
  var image = await fileUpload.save(
    dir == "users" ? request.files.image.data : request.files.data,
  );
  return image;
};
...
const handleImgs = async (request, dir, name, subdir = "") => {
  try {
    ...
    const files = Array.isArray(request.files[name])
      ? request.files[name]
      : [request.files[name]];
...
const handleVideo = async (request, dir, name) => {
  try {
    ...
    const fileName =
      "video" + Math.floor(Math.random() * 100) + Date.now() + ".mp4";
...
const handleUploadTxt = async (req, dir, name) => {
  try {
    ...
    // Generate a unique file name
    const fileName =
      "txt" + Math.floor(Math.random() * 100) + Date.now() + ".txt";
...
const handleUploadAudio = async (req, dir, name) => {
  try {
    ...
    const fileName =
      "audio" + Math.floor(Math.random() * 100) + Date.now() + fileExtension;
...
const handleFileUpdate = async (req, fields, document, deleteType) => {
  ...
  for (const { name, type, dir } of fields) {
    ...
    if (field) {
      const existingFiles = Array.isArray(field)
        ? document[name]
        : [document[name]];
