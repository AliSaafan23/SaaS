...
import { dashboardRoutes, publicRoutes } from "./routes/index.js";
...
function createApp() {
  ...
  const limiter = rateLimit({
    ...
    max: rateLimitMax,
    handler: (req, res) => {
      if (req.path.startsWith("/dashboard") || req.session?.token) {
  ...
  app.use("/dashboard", configureSession("tenant"), dashboardRoutes);
  ...
  app.use((request, response) => {
    ...
    const isStaticAsset =
      /\.(css|js|png|jpg|jpeg|gif|svg|ico|woff|woff2|ttf|eot|map|json|xml|txt)$/i.test(
        requestPath,
      );
    ...
    if (isStaticAsset || isApiRequest || prefersJson) {
      return response.status(404).json({ success: false, key: 'notFound', message: 'Not found', code: 404 });
    }
